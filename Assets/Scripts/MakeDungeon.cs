...
public class MakeDungeon : MonoBehaviour
{
    ...
    */
    ...
    void Start()
    {
        ...
        GameObject startObj = Instantiate(start, new Vector3(startPos[0], startPos[1], 0), Quaternion.identity) as GameObject;
        GameObject goalObj = Instantiate(goal, new Vector3(goalPos[0], goalPos[1], 0), Quaternion.identity) as GameObject;
        startObj.transform.parent = transform;
        goalObj.transform.parent = transform;

        //プレイヤーの生成
        GameObject playerObj = Instantiate(Player, new Vector3(startPos[0], startPos[1], -1), Quaternion.identity) as GameObject;
    }
        ...
        for (int i = -1; i <= max; i++)
        {
            for (int j = -1; j <= max; j++)
            {
                ...
                if (isOutOfRange(i, j)
                    || walls[i, j] == 0)
                {
                    GameObject wallObj = Instantiate(wall, new Vector3(i, j, -1), Quaternion.identity) as GameObject;
                    wallObj.transform.parent = transform;
                }

                //スタートとゴール以外の全ての場所に床オブジェクトを生成
                if(startPos[0] == i && startPos[1] == j)
                {

                }else if(goalPos[0] == i && goalPos[1] == j)
                {

                }else
                {
                    GameObject floorObj = Instantiate(floor, new Vector3(i, j, 0), Quaternion.identity) as GameObject;
